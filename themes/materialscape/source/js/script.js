(function docReady($) {
  ...
  function checkEmail() {
    var $f = $('#Field2');
    if ($f.val().length >= 4 && !$f.hasClass('invalid') && $f.val().indexOf('@') > 0 && $f.val().indexOf('.') > 0 && $f.val().indexOf('.') != $f.val().length - 1) {
      return true;
    }
    return false;
  }
  function checkFields() {
    var ok = $('#Field10').val().length && $('#Field11').val().length && $('#Field1').val().length && checkEmail();
    return ok;
  }
  function checkFieldsEvent(event) {
    var emailOk = checkEmail();
    if (!$(this).val().length || !emailOk) {
  ...
  $('#Field10,#Field11,#Field1').keyup(checkFieldsEvent);
  $('#Field2').keyup(checkFieldsEvent).focusout(checkFieldsEvent);
